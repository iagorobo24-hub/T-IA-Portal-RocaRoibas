...
- [ ] Run against all seven migrated examples and record findings without modifying them. The live acceptance fixture has been checked; the seven-project sweep remains.
...
- [ ] Install or build only after the exact artifact is identified and authorized.
- [ ] Run its doctor/bootstrap command and verify the server starts and publishes its expected tools.
...
- [ ] Write the fixture assertions before scaffold implementation.
- [ ] Verify failure while `tia-create` is absent.
...
- [ ] Add one schema fixture per harness.
- [x] Generate read-only configs first.
- [x] Generate write configs only when explicitly selected.
- [x] Run the verified Claude Code smoke test; Codex/OpenCode remain documented adapters because their local config schema is not fixed here.
